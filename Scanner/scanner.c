	...
	while (1){ /* endless loop broken by token returns it will generate a warning */
		...
		**********************************************************/
		if (c == '.'){
			...
			*/
			...
			t.attribute.err_lex[err_t_ch_index] = '\0';
			return t;
		...
		**********************************************************/
		...
		if (isalpha(c) || isdigit(c)){
			...
			b_addc(lex_buf, '\0'); /* Adds SEOF at the end of the current lexeme to make C-type String */
...
Token aa_func02(char lexeme[]){
	...
	/* If it is not keyword then it would assign to arithmentic variable identifier
	* IF THE lexeme IS LONGER than VID_LEN (see token.h) CHARACTERS,
	* The FIRST VID_LEN means 8 CHARACTERS ARE STORED
	* INTO THE VARIABLE ATTRIBUTE ARRAY vid_lex[] and
	* ADD \0 AT THE END TO MAKE A C-type STRING.
	*/

	t.code = AVID_T; /* Assigning to AVID_T */

	/* Cheking for lexeme size is not more than VID_LEN  */
	if (strlen(lexeme)>VID_LEN){
		for (j = 0; j < VID_LEN; j++){
			t.attribute.vid_lex[j] = lexeme[j]; /* Adding first 8 character */
		}
		t.attribute.vid_lex[j] = '\0'; /* Adding SEOF in the last character */
		return t;
	}
	else{ /* This block will execute when lexeme size is less than 8 */
		for (k = 0; k < strlen(lexeme); k++){
			t.attribute.vid_lex[k] = lexeme[k]; /* Adding lexeme characters */
		}
		t.attribute.vid_lex[k] = '\0';/* Adding SEOF in the last character */
		return t;
	}
}
...
*					(2) if the size of the input (lexeme) is greater than VID_LEN, then only first
*						VID_LEN - 1 characters get stored in the token attribute vid_lex[], '%' at
*						index VID_LEN, and '\0' at index VID_LEN+1 to make it a C-type String.
*					(3)	else all the characters from the input (lexeme) is stored in the the token
*						attribute vid_lex[], '%' at next index, and '\0' at the last index.
*						to make it a C-type String
*					(4) returns the SVID token.
**********************************************************************************************************/
Token aa_func03(char lexeme[]){
	...
	* ONLY FIRST VID_LEN-1 CHARACTERS will be stored
	* into the VARIABLE ATTRIBUTE ARRAY vid_lex[],
	* and THEN THE % CHARACTER added to next character,
	...
	*/
	if (strlen(lexeme) > VID_LEN){
		...
		t.attribute.vid_lex[i++] = '%'; /* Adding character on last second position */
		t.attribute.vid_lex[i] = '\0'; /* Adding on last position */
		return t;
	}
	/* Else lexeme size string will be stored
	* into the variable VARIABLE ATTRIBUTE ARRAY vid_lex[],
	* and THEN THE % CHARACTER added to next character,
	* and ADD \0 AT THE END TO MAKE A C-type STRING.
	*/
	...
	t.attribute.vid_lex[j++] = '%'; /* Adding character on last second position */
	t.attribute.vid_lex[j] = '\0'; /* Adding on last position */
	return t;
}
