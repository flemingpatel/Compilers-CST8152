...
static void st_setsize(void); /* Setting size */
static void st_incoffset(void); /* Incrementing st_offset by 1 */
...
int st_install(STD sym_table, char *lexeme, char type, int line){
	...
	if (is_Found == -1){ /* i.e., lexeme not found in the sym_table */
		...
		for (index = 0; index < length; index++){
			if (b_addc(sym_table.plsBD, lexeme[index]) == NULL){
				return SR_FAIL1;
			}
		...
		return sym_table.st_offset;;		/* returning the index in pstvr array at which the new STVR is stored */
	}
	return is_Found;
}
...
char st_get_type(STD sym_table, int vid_offset){
	...
	if ((sym_table.pstvr[vid_offset].status_field & CHK_SL) == 6){
		return DEFAULT_STRING_I;
	}

	/* Checking for integer vid */
	else if ((sym_table.pstvr[vid_offset].status_field & CHK_IL) == 4){
...
int st_store(STD sym_table){
	...
	for (i = 0; i < sym_table.st_offset; i++){
		...
		if ((st_get_type(sym_table, i)) == 'I'){
			fprintf(fp, "%04X %d %s %d %d", temp->status_field, strlen(temp->plex), temp->plex, temp->o_line, temp->i_value.int_val);
		}
		/* If retur type character is 'F' */
		else if ((st_get_type(sym_table, i)) == 'F'){
			fprintf(fp, "%04X %d %s %d %.2f", temp->status_field, strlen(temp->plex), temp->plex, temp->o_line, temp->i_value.fpl_val);
		}
		/* If retur type character is 'S' */
		else if ((st_get_type(sym_table, i)) == 'S'){
			fprintf(fp, "%04X %d %s %d %d", temp->status_field, strlen(temp->plex), temp->plex, temp->o_line, temp->i_value.str_offset);
		}
