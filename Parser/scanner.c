...
Token aa_func02(char lexeme[]){
	...
	Token t;
	unsigned int i = 0;
	char first_letter_lexeme[VID_LEN + 1];
	unsigned int lengthOfLexeme = strlen(lexeme);
	...
	if (lengthOfLexeme > VID_LEN){
		for (i = 0; i < VID_LEN; i++)
			first_letter_lexeme[i] = lexeme[i];
		first_letter_lexeme[VID_LEN] = '\0';

	}
	else{
		for (i = 0; i < lengthOfLexeme; i++)
			first_letter_lexeme[i] = lexeme[i];
		first_letter_lexeme[i] = '\0';
	}

	/* Checking first letter of lexeme to meet the requirnment of be a an Integer */
	if (first_letter_lexeme[0] == 'i' || first_letter_lexeme[0] == 'o' || first_letter_lexeme[0] == 'd' || first_letter_lexeme[0] == 'w'){
		t.attribute.vid_offset = st_install(sym_table, &first_letter_lexeme[0], DEFAULT_INT_SI, line); /* If it meets than it will be added in symbol table */
	}
	else{ /* Else it would be a float */
		t.attribute.vid_offset = st_install(sym_table, &first_letter_lexeme[0], DEFAULT_FLOAT_SI, line); /* If it meets than it will be added in symbol table */
	}
...
Token aa_func03(char lexeme[]){
	...
	t.attribute.vid_offset = st_install(sym_table, &first_letter_lexeme[0], DEFAULT_STRING_I, line);
