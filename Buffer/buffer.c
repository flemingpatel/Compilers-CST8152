...
Buffer *b_create(short init_capacity, char inc_factor, char o_mode){
	...
	else if (o_mode == 'a' && (unsigned char)inc_factor <= 255 && (unsigned char)inc_factor >= 1){
		tempArray->mode = 1;
...
int b_mode(Buffer * const pBD){
	...
	else if (pBD->mode == 0 || pBD->mode == 1 || pBD->mode == -1){
		return pBD->mode;
	}
	return R_FAIL1;
}
...
int b_print(Buffer * const pBD){
	...
	short temp_getc_offset = pBD->getc_offset; /* holds the initial structure get_offset value */
	int num_character = 0;	/* to keep track how many character are printed */
	unsigned short i = 0;	
	char buffer_character;	/* holds the character from the buffer */
	if (pBD == NULL){
		return R_FAIL1;
	}
	else if (b_isempty(pBD)==1){
		printf("The buffer is empty.\n");
	}
	else{
		pBD->getc_offset = 0;
		for (i = 0; i < (pBD->addc_offset / sizeof(char)); i++) /* Loop go through the last character of buffer */
		{
			if (b_eob(pBD) - 1){
				buffer_character = b_getc(pBD); /* Getting buffer character using b_getc() method and storing in buffer_character */
				printf("%c", buffer_character); /* Printing that character which we just get from the buffer */
				num_character++;
			}
		}
		printf("\n");
...
Buffer *b_pack(Buffer *const pBD){
	...
	n_Capacity = pBD->addc_offset + 1; /* calculating capacity */
	...
	pBD->r_flag = 1; /* Change the flag value because buffer was modified */
	return pBD;
}
/**********************************************************************************************************
